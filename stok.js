...
const Stok = {
    ...
    // 3. PROSES SIMPAN SMART STOK (KASIR/SUPERVISOR AUTO-ISOLASI)
    async simpanSmartStok() {
        ...
        try {
            ...
            if (userRole === 'Owner') {
                ...
                // KASIR/SUPERVISOR: Masuk status isolasi, harga dikunci ke '©©'
                if (produkLama) {
    ...
    async bukaModalEdit(barcode) {
        ...
        const txtIsolasiInfo = document.getElementById('editStokIsolasiInfo');
        if (txtIsolasiInfo) {
    ...
    // 5. HAPUS PRODUK DARI MODAL EDIT (OWNER ONLY)
    async hapusProdukDariModal() {
        if (!this.produkEditAktif) {
            alert("❌ Tidak ada produk yang dipilih untuk dihapus.");
            return;
        }
        await this.hapusProduk(this.produkEditAktif);
        this.tutupModalEdit();
    },

    // 6. HAPUS PRODUK (OWNER ONLY)
    async hapusProduk(barcode) {
        if (typeof Auth !== 'undefined' && Auth.currentUser && Auth.currentUser.role !== 'Owner') {
            alert("⛔ Akses Ditolak! Hanya Owner yang dapat menghapus produk."); return;
        }
        ...
        const hasTransaksi = allTrans.some(t => t.barang && t.barang.some(b => b.barcode === barcode));
        
        let konfirmasiMsg = `⚠️ Hapus produk "${p.nama}" (Barcode: ${p.barcode})?`;
        if (hasTransaksi) {
            konfirmasiMsg += `\n\n🚨 PERINGATAN: Produk ini sudah memiliki riwayat transaksi!\nMenghapus akan membuat data transaksi tidak konsisten.`;
        }
        ...
        try {
            ...
            if (typeof firebase !== 'undefined' && firebase.apps.length) {
                await firebase.database().ref('produk_master/' + barcode).remove();
            }

            alert(`🗑️ Produk "${p.nama}" berhasil dihapus.`);
            this.renderMasterBarang();
        } catch (e) {
    ...
    // 7. DROPDOWN DAN PENGAMAT PINDAH STOK RAK TOKO
    ...
    async pindahKeRakInstan() {
        ...
        // PROTEKSI MUTLAK: Barang yang harganya '©©' diblokir total
        ...
        // KUNCI JUMLAH: Hanya boleh memindahkan stok legal
