// auth.js — Modul Autentikasi Rombakan v2.1 (FIXED: Navigasi Kasir & Supervisor)
const Auth = {
    currentUser: null,
    ...
    async init() {
        ...
        try {
            ...
            if (parsed.type === 'firebase') {
                ...
                this.applyRoleUI(); 
                this.tampilkanApp(); 
                // FIX: Buka tab kasir secara default setelah init
    ...
    // FIX: Hapus class owner-only dari nav-master untuk Kasir/Supervisor
    applyRoleUI() {
        ...
        if (this.isOwner()) {
            ...
            if (btnMaster) {
                btnMaster.classList.remove('hidden');
                btnMaster.classList.remove('owner-only'); // <-- FIX
            }
        }
    },
